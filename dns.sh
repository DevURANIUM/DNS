...
#<title>%s</title><style>%s</style></head><body class="customer">
#<div class="customer-shell"><header><div class="brandline"><span class="brandmark" aria-hidden="true">DNS</span><div><small>DNS / PLAY</small><span>مرکز اتصال بازیکن</span></div></div></header>
#<main class="card">%s</main><footer class="footer-note">DNS / PLAYER HUB</footer></div>%s</body></html>"""
#            % (html.escape(brand()), USER_CSS, inner, UI_SCRIPT))
#
#
#def landing(banner=""):
#    return (banner +
...
#                "برای فعال شدن سرویس، با مدیر تماس بگیرید. "
#                "پس از ثبت پلن، دسترسی شما فعال می‌شود.</div>")
#    if status == "expired":
...
#        body.append("<div class='account-facts'>")
#        for k, v in rows:
#            body.append("<div class='row'><span class='k'>%s</span>"
#                        "<span class='v'>%s</span></div>" % (k, v))
#        body.append("</div>")
#        if quota:
#            body.append("<section class='usage-summary'><div><span>مصرف از سهمیه</span><strong>%d٪</strong></div>%s</section>" % (pct, gauge))
#        body.append(dns_box())
#
#        if not info["ip"]:
#            body.append(
...
#    nav = ""
#    for path, label in (("", "خانه"), ("users", "کاربران"),
...
#<title>%s</title><style>%s</style></head><body class="admin-panel"><div class="wrap">
#<header><div class="brandline"><span class="brandmark" aria-hidden="true">DNS</span><div><small>DNS / CONSOLE</small><h1>%s</h1></div></div><nav aria-label="منوی اصلی">%s<a href='/%s/logout'>خروج</a></nav></header>
#<main>%s%s</main><footer class="footer-note">DNS · مدیریت سرویس و اتصال</footer></div>%s</body></html>""" % (html.escape(title), CSS, html.escape(title), nav,
#                               cfg["ADMIN_PATH"], banner, body, UI_SCRIPT))
#
#
...
#<meta name="viewport" content="width=device-width,initial-scale=1">
#<link rel="icon" href="data:,">
#<title>ورود</title><style>%s</style></head><body class="admin-login"><div class="wrap"><div class="login">
#<div class="card"><div class="brandline"><span class="brandmark" aria-hidden="true">DNS</span><small>DNS</small></div><h2>فرماندهی اتصال</h2><p class="muted">کنترل شبکه و مدیریت بازیکنان، از یک نقطه.</p>%s
#<form method="post" action="/%s/"><div class="f"><label>رمز عبور</label>
#<input type="password" name="password" required autocomplete="current-password" autofocus style="width:100%%"></div>
#<button type="submit" style="width:100%%">ورود</button></form></div>
#</div></div>%s</body></html>""" % (CSS, err, cfg["ADMIN_PATH"], UI_SCRIPT)
#
#
#def bar(used, total):
#    if not total:
...
#        out = ["<section class='welcome'><div><span class='eyebrow'>مرکز مدیریت شبکه</span>"
#               "<h2>همه‌چیز، زیر نظر شما.</h2><p>وضعیت کاربران و مصرف سرویس را مرور کنید و موارد نیازمند بررسی را پیگیری کنید.</p>"
#               "</div><a class='quick-link' href='/%s/users'>مدیریت کاربران ←</a></section>"
#               "<div class='card'><h2>سرویس در یک نگاه</h2><div class='grid'>" % CFG["ADMIN_PATH"]]
...
#        out.append("</div></div>")
#
#        pending = STORE.one("SELECT count(*) c FROM users WHERE status = 'pending'")["c"]
#        limited = STORE.one("SELECT count(*) c FROM users WHERE status IN ('expired', 'over_quota')")["c"]
#        out.append("<div class='card'><h2>نیازمند بررسی</h2><div class='grid'>")
#        for count, label, destination in (
#                (pending, "کاربر در انتظار پلن", "users"),
#                (limited, "حساب نیازمند تمدید", "users")):
#            out.append("<a class='stat' href='/%s/%s'><div class='n'>%d</div>"
#                       "<div class='l'>%s ←</div></a>"
#                       % (CFG["ADMIN_PATH"], destination, count, label))
#        out.append("</div></div>")
#
...
#            quota_gb = str(r["quota_bytes"] / GB) if r["quota_bytes"] else "0"
#            kbps = r["speed_kbps"] or 0
#            speed_mb = ("%g" % (kbps / 1000.0)) if kbps else "0"
#            left = remaining_days(r["expires_at"] or r["quota_reset_at"])
#            username = str(r["username"] or r["phone"] or r["telegram_id"] or "#%d" % r["id"])
#            out.append(
#                "<tr><td class='user-identity' style='min-width:160px;text-align:right'>"
#                "<code class='username-badge'><bdi>%s</bdi></code></td>"
...
#                "<td class='user-template' style='width:220px;min-width:220px'>"
#                "<form method='post' action='/%s/user-template'>"
#                "<input type='hidden' name='id' value='%d'>"
#                "<span class='template-select' style='display:block;width:200px'>"
#                "<select name='template_id' aria-label='قالب کاربر' data-ui='template-select-v2' "
#                "style='width:200px;min-width:200px;max-width:none;min-height:46px;"
#                "padding:10px 14px 10px 42px;direction:rtl;text-align:right;appearance:none' "
#                "onchange='this.form.submit()'>%s</select></span>"
#                "</form></td>"
#                "<td class='%s'>%s</td>"
...
#                elif g.get("note"):
#                    label += "<span class='muted service-note'>%s</span>" % html.escape(g["note"])
...
#                if not 0 <= quota <= 9223372036854775807 or float(gb or 0) < 0 or uid <= 0:
#                    raise ValueError()
#            except (ValueError, OverflowError):
#                return self.redirect("users?m=!عدد سهمیه درست نیست")
#            # Validate every field before writing anything. Previously a bad
#            # speed or expiry still changed the quota and cleared warnings.
#            if "speed_mb" in params:
#                try:
#                    mb = float(one("speed_mb", "0") or 0)
#                    if not math.isfinite(mb) or mb < 0 or mb * 1000 > 9223372036854775807:
#                        raise ValueError()
#                    speed = int(mb * 1000)
#                except (ValueError, OverflowError):
...
#                        raise ValueError()
#                    stamp = (datetime.now(timezone.utc) + timedelta(days=count)).isoformat(timespec="seconds")
#                except (ValueError, OverflowError):
...
#else
#    echo "DNS is not installed on this machine" >&2
#    exit 1
#fi
#
...
#if [ -n "$report" ]; then
#    out="${SMARTDNS_REPORT_DIR:-/tmp}/dns-report-$role-$(date -u +%Y%m%d-%H%M%S).txt"
#    umask 077
#    {
#        echo "DNS report - $role - $(date -u '+%F %T') UTC"
...
#    print("DNS routing - this relay answers as %s" % (me or "?"))
#    print()
...
##!/bin/bash
## smartdns-restart - restart every part of DNS on this machine at once.
##
...
#zoom.us
#
## Additional web and content services
#archive.org
#arcraiders.com
#blender.org
#counter-strike.net
...
#    {
#      "key": "blizzard",
#      "label": "Call of Duty / Warzone / Blizzard",
#      "groups": [
#        {
#          "key": "main",
#          "label": "همه",
...
#    {
#      "key": "riot",
#      "label": "Riot / Valorant / League of Legends",
#      "groups": [
#        {
#          "key": "main",
#          "label": "همه",
...
#            "counter-strike.net"
#          ],
#          "note": "مسیریابی وب و محتوا؛ عملکرد کامل برنامه، تماس، UDP و پورت‌های غیر وب تضمین نمی‌شود. برای ورود و دانلود، Steam را هم انتخاب کنید."
#        }
#      ]
#    },
#    {
...
#    {
#      "key": "fortnite",
#      "label": "Fortnite / Rocket League / Fall Guys",
#      "groups": [
#        {
#          "key": "main",
#          "label": "وب و محتوا",
...
#            "ytimg.com"
#          ],
#          "note": "مسیریابی وب و محتوا؛ عملکرد کامل برنامه، تماس، UDP و پورت‌های غیر وب تضمین نمی‌شود. برای ورود به حساب، Google را هم انتخاب کنید؛ ویدیو ترافیک سرورها را مصرف می‌کند."
#        }
#      ]
#    },
#    {
